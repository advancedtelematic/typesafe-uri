...
private[uri] trait IpAddressParsing { this: Parser ⇒
  import CharacterClasses._

  def `ip-v4-address` = rule {
    `ip-number` ~ '.' ~ `ip-number` ~ '.' ~ `ip-number` ~ '.' ~ `ip-number` ~> (Array[Byte](_,
                                                                                            _,
                                                                                            _,
                                                                                            _))
  }
  ...
  def `ip-v6-address`: Rule1[Array[Byte]] = {
    ...
    def h16(ix: Int) = rule {
      h8(ix) ~ h8(ix + 1) | h4(ix) ~ h8(ix + 1) | zero(ix) ~ h8(ix + 1) | zero(ix) ~ h4(ix + 1)
    }
    def h16c(ix: Int)  = rule { h16(ix) ~ ':' ~ !':' }
    def ch16o(ix: Int) = rule { optional(':' ~ !':') ~ (h16(ix) | zero2(ix)) }
    def ls32 = rule {
      h16(12) ~ ':' ~ h16(14) | `ip-v4-address` ~> (System.arraycopy(_, 0, a, 12, 4))
    }
