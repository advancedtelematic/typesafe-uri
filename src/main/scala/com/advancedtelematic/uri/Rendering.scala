...
/**
  ...
  */
private[uri] trait Renderable {
  private[uri] def render[R <: Rendering](r: R): r.type
}
...
/**
  ...
  */
private[uri] trait ToStringRenderable extends Renderable {
...
/**
  ...
  */
private[uri] trait ValueRenderable extends ToStringRenderable {
...
/**
  ...
  */
private[uri] trait LazyValueBytesRenderable extends Renderable {
...
/**
  ...
  */
private[uri] trait SingletonValueRenderable extends Product with Renderable {
...
/**
  ...
  */
private[uri] trait Renderer[-T] {
  def render[R <: Rendering](r: R, value: T): r.type
}

private[uri] object Renderer {
  ...
  def optionRenderer[D, T](defaultValue: D)(implicit sRenderer: Renderer[D],
  ...
  def genericSeqRenderer[S, T](
      separator: S,
      empty: S
  )(implicit sRenderer: Renderer[S], tRenderer: Renderer[T]): Renderer[immutable.Seq[T]] =
...
/**
  ...
  */
private[uri] trait Rendering {
  def ~~(ch: Char): this.type
  def ~~(bytes: Array[Byte]): this.type

  def ~~(f: Float): this.type  = this ~~ Rendering.floatFormat.format(f.toDouble)
...
/**
  * INTERNAL API
  */
private[uri] class StringRendering extends Rendering {
...
/**
  * INTERNAL API
  */
private[uri] class ByteArrayRendering(sizeHint: Int) extends Rendering {
