...
import com.advancedtelematic.uri.CharacterClasses._
import com.advancedtelematic.uri.Uri._
...
object Uri {
  ...
  /**
    ...
    * If the given string is not a valid URI the method throws an `IllegalUriException`.
    */
  def parseHttpRequestTarget(requestTarget: ParserInput,
                             charset: Charset = UTF8,
                             mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Uri =
  ...
  object Authority {
    ...
    def parse(authorityString: ParserInput,
              charset: Charset = UTF8,
              mode: Uri.ParsingMode = Uri.ParsingMode.Relaxed): Authority =
  ...
  final case class IPv4Host private[uri] (bytes: immutable.Seq[Byte], address: String)
  ...
  object IPv4Host {
    ...
    private[uri] def apply(bytes: Array[Byte], address: String): IPv4Host =
  ...
  object IPv6Host {
    ...
    private[uri] def apply(bytes: String, address: String): IPv6Host = {
      import CharUtils.{ hexValue => hex }
      require(bytes.length == 32, "`bytes` must be a 32 character hex string")
      apply(bytes.toCharArray.grouped(2).map(s ⇒ (hex(s(0)) * 16 + hex(s(1))).toByte).toArray,
            address)
    }
    private[uri] def apply(bytes: Array[Byte], address: String): IPv6Host =
  ...
  object Path {
        ...
        if (ix >= 0)
          if (string.charAt(ix) == '/')
            if (segmentEnd == 0) build(Slash(path), ix - 1)
              ...
              build(Slash(decode(string.substring(ix + 1, segmentEnd), charset) :: path), ix - 1)
    ...
    object /: {
      def unapply(path: Path): Option[(String, Path)] = path match {
        case Segment(head, Slash(tail)) => Some(head -> tail)
        case Segment(head, Empty)       => Some(head -> Empty)
        case Slash(tail)                => Some("/" -> tail)
        case _                          => None
      }
    }
  }
  ...
  sealed abstract class Query
    ...
    def toMultiMap: Map[String, List[String]] = {
      @tailrec def append(map: Map[String, List[String]], q: Query): Map[String, List[String]] =
        if (q.isEmpty) map
        else append(map.updated(q.key, q.value :: map.getOrElse(q.key, Nil)), q.tail)
      append(Map.empty, this)
    }
  ...
  object Query {
    ...
    def apply(input: Option[String], charset: Charset, mode: Uri.ParsingMode): Query =
      input match {
        case None         ⇒ Query.Empty
  ...
  // http://tools.ietf.org/html/rfc3986#section-5.2.2
  private[uri] def resolve(scheme: String,
                           userinfo: String,
                           host: Host,
                           port: Int,
                           path: Path,
    ...
    if (scheme.isEmpty)
      if (host.isEmpty)
        ...
        create(base.scheme, userinfo, host, port, collapseDotSegments(path), query, fragment)
    else create(scheme, userinfo, host, port, collapseDotSegments(path), query, fragment)
  }

  private[uri] def decode(string: String, charset: Charset): String = {
    val ix = string.indexOf('%')
    if (ix >= 0) decode(string, charset, ix)() else string
  }

  @tailrec
  private[uri] def decode(string: String, charset: Charset, ix: Int)(
      sb: JStringBuilder = new JStringBuilder(string.length).append(string, 0, ix)
  ): String =
    if (ix < string.length) string.charAt(ix) match {
      case '%' ⇒
        ...
        while (lastPercentSignIndexPlus3 < string.length && string
                 .charAt(lastPercentSignIndexPlus3) == '%') lastPercentSignIndexPlus3 += 3
  ...
  private[uri] def normalizeScheme(scheme: String): String = {
    @tailrec def verify(ix: Int = scheme.length - 1, allowed: CharPredicate = ALPHA, allLower: Boolean = true)
      : Int =
      if (ix >= 0) {
    ...
    verify() match {
      ...
      case -1 ⇒ scheme
      case ix ⇒
        fail(s"Invalid URI scheme, unexpected character at pos $ix ('${scheme charAt ix}')")
    }
  }

  private[uri] def normalizePort(port: Int, scheme: String): Int =
    if ((port >> 16) == 0)
      if (port != 0 && defaultPorts(scheme) == port) 0 else port
    else fail("Invalid port " + port)

  private[uri] def verifyPath(path: Path, scheme: String, host: Host): Path = {
    if (host.isEmpty) {
  ...
  private[uri] def collapseDotSegments(path: Path): Path = {
  ...
  private[uri] def fail(summary: String, detail: String = "") =
    throw IllegalUriException(summary, detail)

  private[uri] def create(scheme: String,
                          userinfo: String,
                          host: Host,
                          port: Int,
                          path: Path,
...
object UriRendering {
  ...
  def renderQuery[R <: Rendering](r: R,
    ...
    def enc(s: String): r.type = encode(r, s, charset, keep, replaceSpaces = true)
  ...
  private[uri] def encode(r: Rendering,
                          string: String,
                          charset: Charset,
                          keep: CharPredicate,
    ...
    @tailrec def rec(ix: Int): r.type = {
      def appendEncoded(byte: Byte): Unit =
        r ~~ '%' ~~ CharUtils.upperHexDigit(byte >>> 4) ~~ CharUtils.upperHexDigit(byte.toInt)
      if (ix < string.length) {
  ...
  private[uri] def isAsciiCompatible(cs: Charset) = cs == UTF8 || cs == ISO88591 || cs == ASCII
}
