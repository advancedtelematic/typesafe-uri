...
import com.advancedtelematic.uri.Uri.{ Path, _ }
...
class UriSpec extends WordSpec with Matchers {
  ...
  "Uri.Host instances" should {
    ...
    "parse correctly from IPv6 literals (RFC2732)" in {
      ...
      Host("[3ffe:2a00:100:7031::1]") shouldEqual IPv6Host("3ffe2a00010070310000000000000001",
                                                           "3ffe:2a00:100:7031::1")
      Host("[1080::8:800:200C:417A]") shouldEqual IPv6Host("108000000000000000080800200C417A",
                                                           "1080::8:800:200C:417A")
      Host("[::192.9.5.5]") shouldEqual IPv6Host("000000000000000000000000C0090505", "::192.9.5.5")
      Host("[::FFFF:129.144.52.38]") shouldEqual IPv6Host("00000000000000000000FFFF81903426",
                                                          "::FFFF:129.144.52.38")
      ...
      Host("[3:4::5:1.2.3.4]") shouldEqual IPv6Host("00030004000000000000000501020304",
                                                    "3:4::5:1.2.3.4")
      Host("[::ffff:1.2.3.4]") shouldEqual IPv6Host("00000000000000000000ffff01020304",
                                                    "::ffff:1.2.3.4")
      Host("[::0.0.0.0]") shouldEqual IPv6Host("00000000000000000000000000000000", "::0.0.0.0") // Min IPv4
      Host("[::255.255.255.255]") shouldEqual IPv6Host("000000000000000000000000FFFFFFFF",
                                                       "::255.255.255.255") // Max IPv4

      // Zipper position
      Host("[::1:2:3:4:5:6:7]") shouldEqual IPv6Host("00000001000200030004000500060007",
                                                     "::1:2:3:4:5:6:7")
      Host("[1::1:2:3:4:5:6]") shouldEqual IPv6Host("00010000000100020003000400050006",
                                                    "1::1:2:3:4:5:6")
      Host("[1:2::1:2:3:4:5]") shouldEqual IPv6Host("00010002000000010002000300040005",
                                                    "1:2::1:2:3:4:5")
      Host("[1:2:3::1:2:3:4]") shouldEqual IPv6Host("00010002000300000001000200030004",
                                                    "1:2:3::1:2:3:4")
      Host("[1:2:3:4::1:2:3]") shouldEqual IPv6Host("00010002000300040000000100020003",
                                                    "1:2:3:4::1:2:3")
      Host("[1:2:3:4:5::1:2]") shouldEqual IPv6Host("00010002000300040005000000010002",
                                                    "1:2:3:4:5::1:2")
      Host("[1:2:3:4:5:6::1]") shouldEqual IPv6Host("00010002000300040005000600000001",
                                                    "1:2:3:4:5:6::1")
      Host("[1:2:3:4:5:6:7::]") shouldEqual IPv6Host("00010002000300040005000600070000",
                                                     "1:2:3:4:5:6:7::")

      // Zipper length
      Host("[1:1:1::1:1:1:1]") shouldEqual IPv6Host("00010001000100000001000100010001",
                                                    "1:1:1::1:1:1:1")
      Host("[1:1:1::1:1:1]") shouldEqual IPv6Host("00010001000100000000000100010001",
                                                  "1:1:1::1:1:1")
      ...
      Host("[21ff:abcd::1]") shouldEqual IPv6Host("21ffabcd000000000000000000000001",
                                                  "21ff:abcd::1")
      Host("[2001:db8:100:f101::1]") shouldEqual IPv6Host("20010db80100f1010000000000000001",
                                                          "2001:db8:100:f101::1")
      Host("[a:b:c::12:1]") shouldEqual IPv6Host("000a000b000c00000000000000120001", "a:b:c::12:1")
      Host("[a:b::0:1:2:3]") shouldEqual IPv6Host("000a000b000000000000000100020003",
                                                  "a:b::0:1:2:3")
    }
  ...
  "Uri.Path instances" should {
    ...
    "pattern match" in {
      import Path.{ /: }
      Path("/accounts/aaa|bbb") should matchPattern {
        case "/" /: "accounts" /: aaa /: Empty if aaa == "aaa|bbb" =>
      }
    }
  }
  ...
  "Uri.Query instances" should {
    "be parsed correctly in strict mode" in {
      ...
      the[IllegalUriException] thrownBy strict("a%b=c") shouldBe {
        IllegalUriException("Illegal query: Invalid input '=', expected HEXDIG (line 1, column 4)",
                            "a%b=c\n" +
                            "   ^")
      }
      //#query-strict-mode-exception-2
  ...
  "URIs" should {
    ...
    "provide sugar for fluent transformations" in {
      ...
      explicitDefault.withHost(Host("other")) shouldEqual Uri(
      ...
      explicitDefault.withPath(Path("/newpath")) shouldEqual Uri(
        "http://host:80/newpath?query#fragment"
      )

      uri.withUserInfo("someInfo") shouldEqual Uri("http://someInfo@host/path?query#fragment")
      explicitDefault.withUserInfo("someInfo") shouldEqual Uri(
        "http://someInfo@host:80/path?query#fragment"
      )

      uri.withQuery(Query("param1" → "value1")) shouldEqual Uri(
        "http://host/path?param1=value1#fragment"
      )
